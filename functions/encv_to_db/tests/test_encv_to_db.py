...
def test_unknown_field(sample_df: pd.DataFrame) -> None:
    """
    Inserts several rows of sample data into the DB.
    Attempts to handle a field that hasn't been specified in the data model.
    """
    db = SQLiteDB()
    polluted_df = sample_df.copy()
    polluted_df['unknown_field'] = 0
    encv_to_db.push_to_db(db, polluted_df)
    result_df = sqlite_compatible_read(db)
    result_df = result_df[sample_df.columns]  # Reorder columns to match expected data
    assert result_df.equals(sample_df)
